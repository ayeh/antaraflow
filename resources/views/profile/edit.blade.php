...
<div class="max-w-3xl mx-auto space-y-6">
    <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Update your personal information and preferences') }}</p>
    </div>
        ...
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            <form method="POST" action="{{ route('profile.update') }}" class="space-y-4">
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" name="section" value="profile" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Save Changes') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    {{-- Section 2: Avatar --}}
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Avatar') }}</h2>
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            <form method="POST" action="{{ route('profile.avatar') }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <label for="avatar" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Upload new avatar') }}</label>
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Upload Avatar') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        ...
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            <form method="POST" action="{{ route('profile.preferences') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="preferences_theme" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Theme') }}</label>
                        <select name="preferences[theme]" id="preferences_theme" class="w-full rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-4 py-2 text-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none">
                            <option value="system" {{ ($user->preferences['theme'] ?? 'system') === 'system' ? 'selected' : '' }}>{{ __('System') }}</option>
                            <option value="light" {{ ($user->preferences['theme'] ?? 'system') === 'light' ? 'selected' : '' }}>{{ __('Light') }}</option>
                            <option value="dark" {{ ($user->preferences['theme'] ?? 'system') === 'dark' ? 'selected' : '' }}>{{ __('Dark') }}</option>
                        </select>
                    ...
                    <div>
                        <label for="preferences_default_meeting_duration" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Default Meeting Duration (minutes)') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('Notifications') }}</label>
                    <div class="space-y-2">
                        @foreach([
                            'meeting_invite' => __('Meeting Invites'),
                            'action_item_assigned' => __('Action Item Assigned'),
                            'meeting_finalized' => __('Meeting Finalized'),
                        ] as $key => $label)
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" name="section" value="preferences" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Save Preferences') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        ...
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            <form method="POST" action="{{ route('profile.password') }}" class="space-y-4">
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Update Password') }}</button>
                </div>
            </form>
        </div>
    </div>
