...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Meeting Series') }}</h1>
    ...
    @if($series->isEmpty())
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 px-6 py-16 text-center">
            ...
            <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-white">{{ __('No meeting series yet') }}</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Group related meetings into a series to track recurring topics.') }}</p>
            <div class="mt-6">
            ...
            @foreach($series as $item)
                <div class="flex items-center justify-between px-5 py-4 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-1 flex-wrap">
                                @php
                                    ...
                                    $patternLabel = $patternLabels[$item->recurrence_pattern] ?? ucfirst($item->recurrence_pattern ?? __('None'));
                                @endphp
                    ...
                    <div class="flex items-center gap-3 ml-4 flex-shrink-0">
                        <a href="{{ route('meeting-series.edit', $item) }}" class="text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">{{ __('Edit') }}</a>
                        <form method="POST" action="{{ route('meeting-series.destroy', $item) }}" onsubmit="confirmThenSubmit(event, '{{ __('Are you sure you want to delete this series?') }}')">
                            @csrf
                            @method('DELETE')
