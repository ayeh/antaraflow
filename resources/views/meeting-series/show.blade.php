...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div>
                <div class="flex items-center gap-2">
                    ...
                    @php
                        ...
                        $patternLabel = $patternLabels[$meetingSeries->recurrence_pattern] ?? ucfirst($meetingSeries->recurrence_pattern ?? __('None'));
                    @endphp
        ...
        <div class="flex flex-wrap items-center gap-3">
            <a href="{{ route('meeting-series.edit', $meetingSeries) }}" class="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">{{ __('Edit') }}</a>
            <form method="POST" action="{{ route('meeting-series.destroy', $meetingSeries) }}" onsubmit="confirmThenSubmit(event, '{{ __('Are you sure you want to delete this series?') }}')" class="inline">
                @csrf
                @method('DELETE')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
                <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">{{ __('Related Meetings') }}</h2>
                @if($meetingSeries->meetings->isEmpty())
                    <p class="text-sm text-gray-400 dark:text-gray-500">{{ __('No meetings generated yet.') }}</p>
                @else
        ...
        <div class="space-y-4">
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                <h2 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">{{ __('Generate Meetings') }}</h2>
                <form method="POST" action="{{ route('meeting-series.generate', $meetingSeries) }}" class="space-y-3">
                    @csrf
                    <div>
                        <label for="count" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Number of meetings') }}</label>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                ...
                <dl class="space-y-2">
                    <div class="flex justify-between text-sm">
                        <dt class="text-gray-500 dark:text-gray-400">{{ __('Pattern') }}</dt>
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <dd class="font-medium text-gray-800 dark:text-gray-200">{{ $meetingSeries->is_active ? __('Active') : __('Inactive') }}</dd>
                    </div>
                    <div class="flex justify-between text-sm">
