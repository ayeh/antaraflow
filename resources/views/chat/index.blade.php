...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('AI Chat') }} &mdash; {{ $meeting->title }}</h1>
        </div>
    </div>
        ...
        <div x-ref="history" class="flex-1 overflow-y-auto p-6 space-y-4">
            <template x-if="messages.length === 0 && !loading">
                <div class="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-sm">{{ __('No messages yet. Start a conversation about this meeting.') }}</p>
                </div>
            </template>
        ...
        <div class="border-t border-gray-200 dark:border-slate-700 p-4">
            <form @submit.prevent="send()" class="flex gap-3">
                <input
                    ...
                    type="text"
                    placeholder="{{ __('Ask about this meeting...') }}"
                    :disabled="loading"
