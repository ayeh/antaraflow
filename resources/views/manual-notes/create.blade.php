...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Create Manual Note') }}</h1>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <form method="POST" action="{{ route('meetings.manual-notes.store', $meeting) }}" class="space-y-5">
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Save') }}</button>
            </div>
        </form>
    </div>
</div>
