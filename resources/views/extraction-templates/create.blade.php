...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Create Extraction Template') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('extraction-templates.store') }}" class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Leave empty to apply to all meeting types.') }}</p>
            </div>
        </div>
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Optional system-level instruction for the AI model.') }}</p>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
            ...
            <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Create Template') }}</button>
        </div>
    </form>
</div>
@endsection
