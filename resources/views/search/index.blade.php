...
<div class="max-w-4xl mx-auto space-y-6" x-data="{
    ...
    async submitAiSearch() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.aiError = data.message ?? '{{ __('Something went wrong. Please try again.') }}';
            }
        } catch (e) {
    ...
    <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Search across your meetings, action items, and projects.') }}</p>
    </div>
        ...
        @if(isset($results))
            <div class="space-y-6">
                @if(!empty($results['meetings']))
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-3">{{ __('Meetings') }}</h3>
                        <div class="space-y-2">
                ...
                @if(!empty($results['action_items']))
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-3">{{ __('Action Items') }}</h3>
                        <div class="space-y-2">
                ...
                @if(!empty($results['projects']))
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-3">{{ __('Projects') }}</h3>
                        <div class="space-y-2">
                ...
                @if(empty($results['meetings']) && empty($results['action_items']) && empty($results['projects']))
                    <p class="text-sm text-gray-500 dark:text-gray-400 text-center py-8">{{ __('No results found for ":query".', ['query' => $query]) }}</p>
                @endif
            </div>
        @endif
    </div>
    ...
    <div x-show="activeTab === 'ai'" x-cloak class="space-y-4">
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <p class="text-sm text-blue-700 dark:text-blue-300">
                {{ __('Ask a question about any of your meetings. AI will search across all meeting records and provide an answer with sources.') }}
            </p>
        </div>
        ...
        <div class="flex gap-3">
            <textarea
                ...
                placeholder="{{ __('e.g. What decisions were made about the budget? Who was assigned to the marketing task?') }}"
                rows="3"
            ...
            >
                ...
                <span x-text="aiLoading ? '{{ __('Searching...') }}' : '{{ __('Ask AI') }}'"></span>
            </button>
        </div>
        <p class="text-xs text-gray-400 dark:text-gray-500">{{ __('Press ⌘+Enter or Ctrl+Enter to submit') }}</p>
            ...
            <div class="space-y-4">
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">{{ __('AI Answer') }}</span>
                    </div>
                ...
                <template x-if="aiSources.length > 0">
                    <div>
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-3">{{ __('Sources') }}</h3>
                        <div class="space-y-2">
