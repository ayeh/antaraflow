...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                {{ __('Last 6 months') }} &mdash; {{ now()->subMonths(6)->format('M Y') }} to {{ now()->format('M Y') }}
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('Completion Rate') }}</div>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('Overdue Items') }}</div>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 flex items-center gap-4">
            ...
            <div>
                <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($meetingStats['avg_duration_minutes']) }}<span class="text-sm font-normal text-gray-500 dark:text-gray-400 ml-1">{{ __('min') }}</span></div>
                <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('Avg Duration') }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-4 gap-3 pt-2 border-t border-gray-100 dark:border-slate-700">
                    ...
                    <div class="text-center">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('Done') }}</div>
                    </div>
                    <div class="text-center">
                    ...
                    <div class="text-center">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('Overdue') }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('Top Attendees') }}</h2>
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $participationStats['total_attendees'] }} {{ __('unique') }}</span>
            </div>
                ...
                @forelse($participationStats['top_attendees'] as $attendee)
                    <div class="flex items-center justify-between px-6 py-3">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300">
                            {{ $attendee['count'] }} {{ $attendee['count'] === 1 ? __('meeting') : __('meetings') }}
                        </span>
                    </div>
                @empty
                    <div class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('No attendance data available.') }}</div>
                @endforelse
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700">
            <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('AI Usage') }}</h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-200 dark:divide-slate-700">
            <div class="px-6 py-6 flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('Meetings with AI Extraction') }}</div>
                </div>
            </div>
            <div class="px-6 py-6 flex items-center gap-4">
                <div class="bg-violet-100 dark:bg-violet-900/30 rounded-xl p-3 shrink-0">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('AI-Generated Action Items') }}</div>
                </div>
            </div>
        </div>
    </div>
