...
>
    ...
    >
        <div class="flex items-center justify-center gap-2">
            ...
            <span>{{ __("You're offline") }} &mdash; {{ __('Viewing cached data') }}</span>
            <span x-show="pendingCount > 0" class="bg-white/20 rounded-full px-2 py-0.5 text-xs">
                <span x-text="pendingCount"></span> {{ __('pending') }}
            </span>
        </div>
    </div>
    ...
    >
        <div class="flex items-center justify-center gap-3">
            <span>
                <span x-text="pendingCount"></span> {{ __('offline') }}
                <span x-text="pendingCount === 1 ? '{{ __('action') }}' : '{{ __('actions') }}'"></span>
                {{ __('ready to sync') }}
            </span>
            <button
                @click="syncNow()"
            ...
            >
                <span x-show="!isSyncing">{{ __('Sync Now') }}</span>
