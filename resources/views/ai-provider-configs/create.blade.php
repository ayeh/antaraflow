...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900">{{ __('Add Provider') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('ai-provider-configs.store') }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
        ...
        <div>
            <label for="api_key" class="block text-sm font-medium text-gray-700 mb-1">{{ __('API Key') }} <span class="text-red-500">*</span></label>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button type="submit" class="bg-violet-600 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Add Provider') }}</button>
        </div>
    </form>
</div>
@endsection
