...
@section('content')
<h2 class="text-xl font-bold text-gray-900 mb-6">{{ __('auth.register') }}</h2>
...
<form method="POST" action="{{ route('register') }}" class="space-y-4">
    ...
    <button type="submit" class="btn-primary w-full text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">{{ __('auth.register') }}</button>
</form>
...
<p class="mt-4 text-center text-sm text-gray-500">
    {{ __('auth.already_have_account') }} <a href="{{ route('login') }}" class="link-primary font-medium hover:opacity-80">{{ __('auth.login') }}</a>
</p>
@endsection
