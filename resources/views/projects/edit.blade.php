...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Edit Project') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('projects.update', $project) }}" class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 space-y-6">
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
            <a href="{{ route('projects.show', $project) }}" class="px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white">{{ __('Cancel') }}</a>
            <button type="submit" class="bg-violet-600 hover:bg-violet-700 text-white rounded-xl px-6 py-2.5 text-sm font-medium transition-colors">{{ __('Update Project') }}</button>
        </div>
    </form>
</div>
@endsection
