...
<div class="flex flex-col items-center justify-center py-20 text-center" role="status">
    @if($hasFilters)
        ...
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-1">{{ __('No meetings match your filters') }}</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-5 max-w-xs">{{ __('Try adjusting your search or clearing some filters to find what you\'re looking for.') }}</p>
    ...
    @else
        ...
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-1">{{ __('No meetings yet') }}</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-5 max-w-xs">{{ __('Schedule your first meeting to get started with AI-powered transcription and action items.') }}</p>
