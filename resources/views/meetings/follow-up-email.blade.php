...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Follow-up Email') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Review and send follow-up for :title', ['title' => $meeting->title]) }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('meetings.follow-up-email.send', $meeting) }}" class="space-y-6" x-data="{
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('meetings.follow-up-email.generate', $meeting) }}" class="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">{{ __('Regenerate') }}</a>
