...
<div class="space-y-6">
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Meeting Information') }}</h2>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Meeting Date') }}</p>
                <p class="text-base font-medium text-gray-900 dark:text-white">
                    {{ $meeting->meeting_date ? $meeting->meeting_date->format('d M Y') : '-' }}
                </p>
            </div>
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Project') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Prepared By') }}</p>
                <p class="text-base font-medium text-gray-900 dark:text-white">
                    {{ $meeting->prepared_by ?? $meeting->createdBy->name }}
                </p>
            </div>
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Language') }}</p>
                <p class="text-base font-medium text-gray-900 dark:text-white">
                    {{ $meeting->language === 'ms' ? 'Bahasa Melayu' : 'English' }}
                </p>
            </div>
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('MOM Number') }}</p>
        ...
        @if($meeting->start_time || $meeting->end_time || $meeting->duration_minutes)
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                @if($meeting->start_time)
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Start Time') }}</p>
                        <p class="text-base font-medium text-gray-900 dark:text-white">
                ...
                @if($meeting->end_time)
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('End Time') }}</p>
                        <p class="text-base font-medium text-gray-900 dark:text-white">
                ...
                @if($meeting->duration_minutes)
                    <div>
                        ...
                        <p class="text-base font-medium text-gray-900 dark:text-white">
                            {{ $meeting->duration_minutes }} {{ __('minutes') }}
                        </p>
                    </div>
                @endif
            </div>
        ...
        @if($meeting->meeting_link)
            <div class="mt-4 flex items-center gap-3">
                <div class="flex-1">
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Meeting Link') }}</p>
    ...
    @if($meeting->topics->isNotEmpty())
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">{{ __('Agenda') }}</h2>
            <ol class="space-y-2">
    ...
    @if($meeting->tags->isNotEmpty())
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">{{ __('Tags') }}</h2>
    ...
    <div class="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-5 py-3 text-sm">
        <div class="flex items-baseline gap-1.5">
            ...
            <span class="text-gray-500 dark:text-gray-400">{{ __('Attendees') }}</span>
            <span class="text-xs text-gray-400 dark:text-gray-500">· {{ $attendeeStats['present'] }} {{ __('present') }}</span>
        </div>
        ...
        <div class="flex items-baseline gap-1.5">
            ...
            <span class="text-gray-500 dark:text-gray-400">{{ __('Action items') }}</span>
            <span class="text-xs text-gray-400 dark:text-gray-500">· {{ $actionItemStats['completed'] }} {{ __('completed') }}</span>
        </div>
        <span class="hidden sm:block h-4 w-px bg-gray-200 dark:bg-slate-700"></span>
        <div class="flex items-baseline gap-1.5">
            <span class="text-lg font-semibold text-blue-600 dark:text-blue-400">{{ $actionItemStats['in_progress'] }}</span>
            <span class="text-gray-500 dark:text-gray-400">{{ __('In Progress') }}</span>
        </div>
        <span class="hidden sm:block h-4 w-px bg-gray-200 dark:bg-slate-700"></span>
        <div class="flex items-baseline gap-1.5">
            ...
            <span class="text-gray-500 dark:text-gray-400">{{ __('Overdue') }}</span>
        </div>
    </div>
</div>
