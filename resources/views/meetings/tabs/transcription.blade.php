<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h3 class="text-lg font-medium text-gray-900">{{ __('Transcriptions') }}</h3>
    </div>
            ...
            @foreach($transcriptions as $transcription)
                <div class="py-4">
                    <div class="flex items-center justify-between mb-2">
                        <div class="text-sm font-medium text-gray-900">{{ $transcription->original_filename ?? __('Transcription') }}</div>
    ...
    @else
        <p class="text-sm text-gray-500 text-center py-8">{{ __('No transcriptions yet. Upload an audio file to get started.') }}</p>
    @endif
</div>
