<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h3 class="text-lg font-medium text-gray-900">{{ __('AI Insights') }}</h3>
        <form method="POST" action="{{ route('meetings.extract', $meeting) }}" class="inline">
            @csrf
            <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Run AI Extraction') }}</button>
        </form>
    </div>
    ...
    @if($extractions->isNotEmpty())
        @foreach($extractions as $extraction)
            <div class="border border-gray-200 rounded-lg p-4 space-y-4">
                ...
                @if($extraction->summary)
                    <div>
                        <h4 class="text-sm font-medium text-gray-700 mb-1">{{ __('Summary') }}</h4>
                ...
                @if($extraction->extracted_data)
                    @if(isset($extraction->extracted_data['action_items']))
                        <div>
                            <h4 class="text-sm font-medium text-gray-700 mb-1">{{ __('Action Items') }}</h4>
                    ...
                    @if(isset($extraction->extracted_data['decisions']))
                        <div>
                            <h4 class="text-sm font-medium text-gray-700 mb-1">{{ __('Decisions') }}</h4>
                            <ul class="list-disc list-inside text-sm text-gray-600 space-y-1">
        ...
        @endforeach
    @else
        <p class="text-sm text-gray-500 text-center py-8">{{ __('No AI extractions yet. Click "Run AI Extraction" to analyze the meeting content.') }}</p>
    @endif
    ...
    @if($risksExtraction && !empty($risksExtraction->structured_data) && !isset($risksExtraction->structured_data['custom_template']))
        <div class="border border-gray-200 rounded-lg p-4 space-y-3">
            <h4 class="text-sm font-medium text-gray-700">{{ __('Risks & Concerns') }}</h4>
            <div class="space-y-2">
                    ...
                    <div class="flex items-start gap-3 p-3 rounded-lg {{ match($risk['severity'] ?? 'medium') { 'high' => 'bg-red-50', 'medium' => 'bg-amber-50', 'low' => 'bg-yellow-50', default => 'bg-gray-50' } }}">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if(!empty($risk['mitigation']))
                                <p class="text-xs text-gray-500 mt-1"><span class="font-medium">{{ __('Mitigation:') }}</span> {{ $risk['mitigation'] }}</p>
                            @endif
                            @if(!empty($risk['raised_by']))
                                <p class="text-xs text-gray-400 mt-0.5">{{ __('Raised by') }} {{ $risk['raised_by'] }}</p>
                            @endif
                        </div>
                    </div>
                @endforeach
    ...
    @if($topics->isNotEmpty())
        <div>
            <h4 class="text-sm font-medium text-gray-700 mb-2">{{ __('Topics') }}</h4>
