    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-gray-900">{{ __('Version') }} {{ $version->version_number }}</h1>
    ...
    <div class="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
        ...
        <div class="p-6 space-y-3">
            <div class="flex items-center justify-between">
                <div class="space-y-1">
                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">{{ __('Change Summary') }}</p>
                    <p class="text-sm text-gray-900">{{ $version->change_summary ?? __('Auto-saved') }}</p>
                </div>
                @if(!$isLatest)
        ...
        <div class="p-6 space-y-4">
            <div>
                <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">{{ __('Title') }}</p>
                <p class="text-base font-semibold text-gray-900">{{ $version->snapshot['title'] ?? '—' }}</p>
            </div>

            @if(!empty($version->snapshot['content']))
                <div>
                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">{{ __('Content') }}</p>
            ...
            @else
                <div class="bg-gray-50 rounded-lg p-4 text-sm text-gray-400 italic">
                    {{ __('No content recorded for this version.') }}
                </div>
            @endif
        </div>
    </div>
