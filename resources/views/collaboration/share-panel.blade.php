<div class="space-y-6" x-data="{ isLinkShare: false }">
    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Sharing') }}</h2>
        ...
        <div class="overflow-x-auto border border-gray-200 dark:border-slate-700 rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-800">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Shared With') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Permission') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Expires') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Shared By') }}</th>
                    ...
                    @foreach($shares as $share)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                                {{ $share->expires_at ? $share->expires_at->format('M j, Y') : __('Never') }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                                {{ $share->sharedBy?->name ?? '—' }}
                            </td>
                            <td class="px-4 py-3 text-right">
                                <form method="POST" action="{{ route('meetings.shares.destroy', [$meeting, $share]) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-sm text-red-600 dark:text-red-400 hover:underline" onclick="event.preventDefault(); window.antaraConfirm('{{ __('Revoke this share?') }}').then(ok => ok && this.closest('form').submit())">{{ __('Revoke') }}</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
    ...
    @else
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('This meeting has not been shared yet.') }}</p>
    @endif
    ...
    @if($guestShare)
        <div class="border border-blue-200 dark:border-blue-800 rounded-lg p-4 bg-blue-50 dark:bg-blue-950">
            <h3 class="text-sm font-semibold text-blue-800 dark:text-blue-200 mb-2">{{ __('Shareable Link') }}</h3>
            <p class="text-xs text-blue-600 dark:text-blue-400 mb-3">{{ __('Anyone with this link can view this meeting (read-only).') }}</p>
    ...
    <div class="border border-gray-200 dark:border-slate-700 rounded-lg p-4 bg-gray-50 dark:bg-slate-800">
        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">{{ __('Add New Share') }}</h3>
        <form method="POST" action="{{ route('meetings.shares.store', $meeting) }}" class="space-y-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <label for="is_link_share" class="text-sm text-gray-700 dark:text-gray-300">{{ __('Generate shareable link (no specific user)') }}</label>
            </div>

            <div x-show="!isLinkShare" x-cloak>
                <label for="shared_with_user_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Share With User') }}</label>
            ...
            <div>
                <label for="permission" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Permission') }}</label>
                <select id="permission" name="permission" required class="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <option value="comment">{{ __('Comments') }}</option>
                    <option value="edit">{{ __('Edit') }}</option>
                </select>
            </div>

            <div>
                <label for="expires_at" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Expiry Date') }} <span class="text-gray-400">{{ __('(optional)') }}</span></label>
            ...
            <div>
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">{{ __('Share') }}</button>
            </div>
        </form>
    </div>
</div>
