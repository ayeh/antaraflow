...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Organization Settings') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage settings for :name', ['name' => $organization->name]) }}</p>
        </div>
    </div>

    {{-- Section 1: General Settings --}}
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('General Settings') }}</h2>
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            <form method="POST" action="{{ route('organizations.settings.update', $organization) }}" class="space-y-4">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Organization Name') }} <span class="text-red-500">*</span></label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="timezone" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Timezone') }} <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label for="language" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Language') }} <span class="text-red-500">*</span></label>
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Save Settings') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Organization Logo') }}</h2>
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            <form method="POST" action="{{ route('organizations.settings.logo', $organization) }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <label for="logo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Upload new logo') }}</label>
                        <input type="file" name="logo" id="logo" accept="image/*"
                            class="w-full rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-4 py-2 text-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none">
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Max 2MB. Supported formats: JPG, PNG, GIF, WebP.') }}</p>
                        @error('logo')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Upload Logo') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        ...
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            <form method="POST" action="{{ route('organizations.settings.update', $organization) }}" class="space-y-4">
                ...
                <div>
                    <label for="teams_webhook_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Microsoft Teams Webhook URL') }}</label>
                    <input type="url" name="teams_webhook_url" id="teams_webhook_url" value="{{ old('teams_webhook_url', $organization->teams_webhook_url) }}"
                        ...
                        class="w-full rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-4 py-2 text-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none">
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Add an Incoming Webhook URL from your Teams channel to receive meeting notifications.') }}</p>
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Save Integrations') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Cost Analytics') }}</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Configure hourly rates per role for meeting cost calculations.') }}</p>
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            ...
            <form method="POST" action="{{ route('organizations.settings.update', $organization) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label for="hourly_rate_admin" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Admin Rate ($/hr)') }}</label>
                    ...
                    <div>
                        <label for="hourly_rate_manager" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Manager Rate ($/hr)') }}</label>
                    ...
                    <div>
                        <label for="hourly_rate_member" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Member Rate ($/hr)') }}</label>
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 dark:border-slate-700">
                    <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Save Rates') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('organizations.members.index', $organization) }}" class="text-sm text-violet-600 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-300 font-medium transition-colors">{{ __('Manage Members') }}</a>
        </div>
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            @if($members->isEmpty())
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('No members found.') }}</p>
            @else
                <ul class="space-y-3">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Subscription') }}</h2>
        <div class="border-t border-gray-200 dark:border-slate-700 mt-4 pt-4">
            @if($subscription)
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {{ $subscription->subscriptionPlan->name ?? __('Current Plan') }}
                        </p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                            {{ __('Status:') }} <span class="font-medium capitalize">{{ $subscription->status }}</span>
                            @if($subscription->ends_at)
                                &middot; {{ __('Renews') }} {{ $subscription->ends_at->format('M j, Y') }}
                            @endif
                        </p>
                    </div>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
            ...
            @else
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('You are currently on the free plan.') }}</p>
                    </div>
