...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Create Organization') }}</h1>
    </div>
        ...
        <form method="POST" action="{{ route('organizations.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    class="w-full rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-4 py-2 text-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none resize-none">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
                <a href="{{ route('organizations.index') }}" class="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">{{ __('Cancel') }}</a>
                <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Create Organization') }}</button>
            </div>
        </form>
    </div>
</div>
