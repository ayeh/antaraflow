...
$navItems = [
    ...
    [
        ...
        'badge'  => \App\Domain\AI\Models\ProactiveInsight::query()->active()->unread()->count(),
    ],
    [
        'key'    => 'reports',
        ...
        <div class="relative group">
            ...
            >
                ...
                <span
                    ...
                    x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0"
                    class="ml-auto inline-flex items-center justify-center px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-red-500 text-white min-w-[18px]"
                >{{ $item['badge'] }}</span>
                @endif
            </a>
            @else
        ...
        <div class="space-y-0.5">
            @foreach(config('locales.supported', []) as $code => $label)
                <a href="{{ route('locale.switch', $code) }}"
                   class="flex items-center justify-between gap-3 px-3 py-2 rounded-xl text-sm transition-colors
                          {{ app()->getLocale() === $code ? 'bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 font-medium' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' }}">
                    <span>{{ $label }}</span>
