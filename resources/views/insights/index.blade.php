...
<div class="space-y-6" x-data="{ typeFilter: '' }">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Insights') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ __('Proactive insights from MemoAdvisor') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4">
        <div class="flex flex-wrap items-center gap-4">
            <div>
                <label for="type_filter" class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{{ __('Filter by Type') }}</label>
                <select id="type_filter" x-model="typeFilter"
                    ...
                    <option value="recurring_topic">{{ __('Recurring Topic') }}</option>
                    <option value="decision_no_followup">{{ __('Decision No Follow-up') }}</option>
                    <option value="reraised_action_item">{{ __('Reraised Action Item') }}</option>
                    <option value="overdue_pattern">{{ __('Overdue Pattern') }}</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            @if($insight->severity === 'critical')
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300">{{ __('Critical') }}</span>
                            @elseif($insight->severity === 'warning')
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">{{ __('Warning') }}</span>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">{{ __('Info') }}</span>
                            @endif
                            ...
                            @if(!$insight->is_read)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300">{{ __('New') }}</span>
                            @endif
                        </div>
        ...
        @empty
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
                ...
                <p class="mt-3 text-sm font-medium text-gray-900 dark:text-white">{{ __('No insights yet') }}</p>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('MemoAdvisor will generate insights as more meeting data is available.') }}</p>
            </div>
        @endforelse
    </div>
