...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage PDF and document export templates for your organization') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        @if($templates->isEmpty())
            <div class="px-6 py-12 text-center text-gray-500 dark:text-gray-400 text-sm">
                {{ __('No export templates yet.') }} <a href="{{ route('settings.export-templates.create') }}" class="text-violet-600 hover:underline">{{ __('Create your first template') }}</a>.
            </div>
        @else
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-slate-700">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Primary Color') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Font') }}</th>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 text-right space-x-3">
                                <a href="{{ route('settings.export-templates.edit', $template) }}"
                                    class="text-violet-600 hover:text-violet-700 text-sm font-medium">{{ __('Edit') }}</a>
