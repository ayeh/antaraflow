...
<div class="max-w-4xl mx-auto space-y-6">
    <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage API keys to access your organization\'s data programmatically') }}</p>
    </div>
                ...
                <div class="flex flex-wrap gap-4">
                    @foreach(['read', 'write', 'delete'] as $permission)
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            >
                            <span class="text-sm text-gray-700 dark:text-gray-300 capitalize">{{ __(ucfirst($permission)) }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
                    ...
                    @foreach($apiKeys as $apiKey)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <form method="POST" action="{{ route('settings.api-keys.destroy', $apiKey) }}" onsubmit="confirmThenSubmit(event, '{{ __('Revoke this API key? This cannot be undone.') }}')">
                                    @csrf
                                    @method('DELETE')
