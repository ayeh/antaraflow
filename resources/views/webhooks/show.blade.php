...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Webhook Deliveries') }}</h1>
        ...
        <form method="POST" action="{{ route('webhooks.ping', $webhook) }}" class="inline">
            @csrf
            <button type="submit" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Send Test Ping') }}</button>
        </form>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            ...
            <div>
                <p class="text-gray-500 dark:text-gray-400">{{ __('Failures') }}</p>
                <p class="font-medium text-gray-900 dark:text-white">{{ $webhook->failure_count }}</p>
            </div>
            <div>
                <p class="text-gray-500 dark:text-gray-400">{{ __('Events') }}</p>
    ...
    @if($deliveries->isEmpty())
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
            <p class="text-gray-500 dark:text-gray-400">{{ __('No deliveries recorded yet.') }}</p>
        </div>
    @else
        <div class="space-y-3">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4" x-data="{ open: false }">
                    <div class="flex items-center justify-between cursor-pointer" @click="open = !open">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-xs text-gray-500 dark:text-gray-400">
                                {{ $delivery->response_status ? "HTTP {$delivery->response_status}" : __('Connection Error') }}
                            </span>
                            <span class="text-xs text-gray-400 dark:text-gray-500">{{ __('Attempt') }} {{ $delivery->attempt }}</span>
                        </div>
                    ...
                    <div x-show="open" x-cloak class="mt-3 pt-3 border-t border-gray-200 dark:border-slate-700">
                        ...
                        @if($delivery->response_body)
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">{{ __('Response') }}: {{ Str::limit($delivery->response_body, 200) }}</p>
                        @endif
                    </div>
                </div>
            @endforeach
