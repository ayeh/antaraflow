...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage outgoing webhook endpoints for event notifications') }}</p>
        </div>
        @can('create', \App\Domain\Webhook\Models\WebhookEndpoint::class)
            <a href="{{ route('webhooks.create') }}" class="bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">{{ __('Add Endpoint') }}</a>
        @endcan
    </div>
    ...
    @if($endpoints->isEmpty())
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400">{{ __('No webhook endpoints configured yet.') }}</p>
        </div>
    @else
        <div class="space-y-4">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                    <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                @if($endpoint->failure_count > 0)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300">
                                        {{ $endpoint->failure_count }} {{ __('failure(s)') }}
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="flex items-center gap-2 ml-4">
                            <a href="{{ route('webhooks.show', $webhook = $endpoint) }}" class="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">{{ __('Deliveries') }}</a>
                            @can('update', $endpoint)
                                <a href="{{ route('webhooks.edit', $webhook = $endpoint) }}" class="text-sm text-violet-600 dark:text-violet-400 hover:text-violet-700">{{ __('Edit') }}</a>
                            @endcan
                            @can('delete', $endpoint)
                                <form method="POST" action="{{ route('webhooks.destroy', $webhook = $endpoint) }}" class="inline" onsubmit="confirmThenSubmit(event, 'Delete this webhook endpoint?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-sm text-red-600 dark:text-red-400 hover:text-red-700">{{ __('Delete') }}</button>
                                </form>
                            @endcan
                        </div>
                    </div>
