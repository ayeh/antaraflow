...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __("Sub-Organizations") }}</h1>
        </div>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg px-4 py-3">
        <p class="text-sm text-blue-700 dark:text-blue-300">
            {{ $subOrganizations->count() }} of {{ $resellerSetting->max_sub_organizations }} {{ __('sub-organizations used.') }}
        </p>
    </div>
    @endif
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __("Create Sub-Organization") }}</h2>
        <form action="{{ route('reseller.sub-organizations.store') }}" method="POST" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __("Organization Name") }}</label>
                ...
                <div>
                    <label for="owner_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __("Owner Name") }}</label>
                ...
                <div>
                    <label for="owner_email" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __("Owner Email") }}</label>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('reseller.sub-organizations') }}" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">{{ __("Cancel") }}</a>
    ...
    @if($subOrganizations->isEmpty())
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
            ...
            <h3 class="mt-3 text-sm font-medium text-gray-900 dark:text-white">{{ __("No sub-organizations") }}</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Get started by creating your first sub-organization.') }}</p>
        </div>
    @else
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-700/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __("Members") }}</th>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700 dark:bg-slate-700 dark:text-gray-300">
                                {{ $sub?->subscriptionPlan?->name ?? __('Free') }}
                            </span>
                        </td>
