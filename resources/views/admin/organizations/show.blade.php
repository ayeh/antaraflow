...
@section('breadcrumbs')
    <nav class="text-sm text-slate-400 mb-1">
        ...
        <a href="{{ route('admin.organizations.index') }}" class="hover:text-white">{{ __('Organizations') }}</a>
        <span class="mx-1">/</span>
...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 bg-slate-800 border border-slate-700 rounded-xl p-6">
            <div class="flex items-start justify-between">
                ...
                @if($organization->is_suspended)
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-900/30 text-red-400 border border-red-800">{{ __('Suspended') }}</span>
                @else
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-900/30 text-green-400 border border-green-800">{{ __('Active') }}</span>
                @endif
            </div>
            ...
            @if($organization->is_suspended && $organization->suspended_reason)
                <div class="mt-4 bg-red-900/20 border border-red-800 rounded-lg px-4 py-3">
                    <span class="text-xs text-red-400 uppercase tracking-wider font-medium">{{ __('Suspension Reason') }}</span>
                    <p class="text-red-300 text-sm mt-1">{{ $organization->suspended_reason }}</p>
                    @if($organization->suspended_at)
                        <p class="text-red-400/60 text-xs mt-2">{{ __('Suspended on') }} {{ $organization->suspended_at->format('M d, Y H:i') }}</p>
                    @endif
                </div>
            @endif
            ...
            <div class="grid grid-cols-2 gap-4 mt-6">
                <div>
                    <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('Created') }}</span>
                    <p class="text-slate-200 mt-1">{{ $organization->created_at->format('M d, Y') }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('Timezone') }}</span>
                    <p class="text-slate-200 mt-1">{{ $organization->timezone ?? 'UTC' }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('Language') }}</span>
                    <p class="text-slate-200 mt-1">{{ strtoupper($organization->language ?? 'en') }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('SMTP Config') }}</span>
                    <p class="text-slate-200 mt-1">
                        @if($hasSmtpConfig)
                            <span class="text-green-400">{{ __('Configured') }}</span>
                        @else
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-xl p-6">
            <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('Current Plan') }}</span>
            ...
            @else
                <p class="text-2xl font-bold text-slate-500 mt-2">{{ __('No Plan') }}</p>
                <p class="text-slate-500 text-sm mt-1">{{ __('No active subscription') }}</p>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-slate-800 border border-slate-700 rounded-xl p-6">
            <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('Total Members') }}</span>
            <p class="text-3xl font-bold text-white mt-2">{{ number_format($organization->members->count()) }}</p>
        </div>
        <div class="bg-slate-800 border border-slate-700 rounded-xl p-6">
            <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('Total Meetings') }}</span>
            <p class="text-3xl font-bold text-white mt-2">{{ number_format($totalMeetings) }}</p>
            <p class="text-slate-500 text-xs mt-1">{{ number_format($meetingsLast30Days) }} {{ __('in last 30 days') }}</p>
        </div>
        <div class="bg-slate-800 border border-slate-700 rounded-xl p-6">
            <span class="text-xs text-slate-500 uppercase tracking-wider">{{ __('Storage Used') }}</span>
    ...
    <div class="flex flex-wrap items-start gap-4 mb-8">
        ...
        @else
            <form method="POST" action="{{ route('admin.organizations.suspend', $organization) }}" class="flex items-end gap-2"
                ...
                @csrf
                <div>
                    <label for="reason" class="block text-xs text-slate-400 mb-1">{{ __('Suspension Reason') }}</label>
                    <input type="text"
                           name="reason"
                           id="reason"
                           required
        ...
        <form method="POST" action="{{ route('admin.organizations.change-plan', $organization) }}" class="flex items-end gap-2">
            ...
            @method('PUT')
            <div>
                <label for="plan_id" class="block text-xs text-slate-400 mb-1">{{ __('Change Plan') }}</label>
                <select name="plan_id"
                        id="plan_id"
                        required
    ...
    <div class="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-700">
            <h3 class="text-sm font-semibold text-white uppercase tracking-wider">{{ __('Members') }}</h3>
        </div>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-slate-400">
                            {{ __('No members in this organization.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
