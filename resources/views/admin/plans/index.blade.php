...
@section('breadcrumbs')
    <nav class="text-sm text-slate-400 mb-1">
        ...
        <span class="text-slate-200">{{ __('Subscription Plans') }}</span>
    </nav>
@endsection

@section('content')
    <div class="flex justify-between items-center mb-6">
        <p class="text-slate-400 text-sm">{{ __('Manage subscription plans and pricing.') }}</p>
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="bg-slate-700/50 text-slate-300 text-xs uppercase tracking-wider">
                    <th class="px-6 py-3">{{ __('Sort') }}</th>
                    <th class="px-6 py-3">{{ __('Name') }}</th>
                    <th class="px-6 py-3">{{ __('Slug') }}</th>
                    <th class="px-6 py-3">{{ __('Monthly Price') }}</th>
                    <th class="px-6 py-3">{{ __('Yearly Price') }}</th>
                    <th class="px-6 py-3">{{ __('Subscribers') }}</th>
                ...
                @forelse($plans as $plan)
                    <tr class="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-700 text-slate-400 border border-slate-600">{{ __('Inactive') }}</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.plans.edit', $plan) }}"
                                   class="text-sm text-blue-400 hover:text-blue-300 transition-colors">{{ __('Edit') }}</a>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center text-slate-400">
                            {{ __('No subscription plans found. Create your first plan to get started.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
