...
@section('breadcrumbs')
    <nav class="text-sm text-slate-400 mb-1">
        ...
        <span class="text-slate-200">{{ __('Users') }}</span>
    </nav>
@endsection
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="bg-slate-700/50 text-slate-300 text-xs uppercase tracking-wider">
                    ...
                    <th class="px-6 py-3">{{ __('Organizations') }}</th>
            ...
            <tbody class="divide-y divide-slate-700">
                @forelse($users as $user)
                    <tr class="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-6 py-4 text-slate-400">{{ $user->last_login_at?->format('Y-m-d H:i') ?? __('Never') }}</td>
                        <td class="px-6 py-4">
                            @if($user->trashed())
                        ...
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('admin.users.show', $user) }}"
                               class="text-sm text-blue-400 hover:text-blue-300 transition-colors">{{ __('View') }}</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                            @if(request('search'))
                                {{ __('No users found matching') }} "{{ request('search') }}".
                            @else
                                {{ __('No users found.') }}
                            @endif
                        </td>
