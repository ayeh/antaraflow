...
<div class="max-w-3xl mx-auto space-y-6" x-data="sectionBuilder({{ Js::from($initialSections) }})">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Create Template') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('meeting-templates.store') }}" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 space-y-5">
            ...
            {{-- Quick-start presets --}}
            <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-wide">{{ __('Quick-start presets') }}</p>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('meeting-templates.index') }}" class="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">{{ __('Cancel') }}</a>
            <button type="submit"
