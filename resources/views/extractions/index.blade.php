...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Extractions for :title', ['title' => $meeting->title]) }}</h1>
        </div>
    </div>
    ...
    @empty
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 text-sm">{{ __('No extractions yet.') }}</p>
        </div>
    @endforelse

    @if($topics->isNotEmpty())
        <div>
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Topics') }}</h2>
